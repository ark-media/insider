...
export function FAQ({ faqs }: { faqs: Faq[] }) {
  // Open state is keyed by FAQ id, not list index — grouping means an index is
  // no longer unique across sections.
      ...
      <div className="page-section">
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-12">
          ...
          <div className="lg:col-span-8">
            {groups.map((group, gi) => (
              <div key={group.category || `__ungrouped-${gi}`}>
                {group.category ? (
                  <h3 className="display-upright mt-12 text-[clamp(1.2rem,2vw,1.6rem)] text-fg-strong first:mt-0">
                    {group.category}
                  </h3>
                ) : null}
                <ul
                  className={`border-t border-rule ${group.category ? "mt-5" : ""}`}
                >
                  {group.items.map((f, i) => {
