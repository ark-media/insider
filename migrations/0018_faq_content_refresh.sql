-- 0018_faq_content_refresh.sql
-- Replaces the FAQ set seeded by 0014 with the revised launch copy. Same shape
-- (five sections, a global 1..N display_order; the component derives section
-- order from first appearance), so this is a content-only refresh.
--
-- What changed vs 0014:
--   * Section 1 renamed to "I am already subscribed to Inside Call Me Back.
--     What does this mean for me?" and its two questions swapped order; the
--     weekly series is now named Call Me Back AMA throughout.
--   * Rewritten answers: Ark+ includes, website-vs-Apple difference, sharing,
--     listening in other apps, adding feeds, "Apple says I'm not subscribed",
--     "I swear I'm subscribed on Spotify", new phone, pay through Spotify.
--   * Added: "Why is Spotify asking me to connect my account?"
--   * Removed: "Why can't I see subscriber-only episodes?"
--
-- Like 0014 this wipes prior FAQ rows and reseeds from scratch — pre-launch and
-- greenfield, so a clean replace is the intended behaviour. Any edits made in
-- the admin back office since 0014 are discarded.
--
...
values
  ...
  (
    ...
    $A$<p>No. If you already had an active Inside Call Me Back subscription, your subscription has automatically become an Ark+ subscription. You do not need to subscribe again to continue receiving your podcast benefits. If you'd like access to the Community app, you can upgrade to the Bundle at any time.</p>$A$
  ),
  (
    2,
    $C$I am already subscribed to Inside Call Me Back. What does this mean for me?$C$,
  ...
  (
    ...
    $A$<p>Ark+ subscriptions are tied to a single account and can't be shared across household members or multiple devices with separate logins. Each person who wants subscriber-only episodes, early access, and ad-free listening, or Community access will need their own subscription.</p>$A$
  ),
  ...
  (
    ...
    $A$<p>If you purchased an Ark+ subscription through the Ark Media website, or were previously an Inside Call Me Back subscriber, your subscription is managed through Ark Media, not through Apple Podcasts.</p><p>Because of this, Apple Podcasts won't show you as an Apple subscriber, even though your Ark+ subscription is active.</p><p>To access subscriber-only episodes and listen ad-free, sign in to your Ark Media account and go to the Set Up Feed page. From there, add your Ark+ feeds to your preferred podcast app. Once you've added them, follow or favorite them so new episodes appear automatically.</p>$A$
  ),
  (
    20,
    $C$Getting Started$C$,
  ...
  (
    ...
    $A$<p>No. Your Ark+ subscription remains active when you get a new phone.</p><p>If your subscriber content doesn't appear automatically, sign in to your Ark Media account on your new device and reconnect your preferred podcast app from the Set Up Feed page.</p>$A$
  ),
