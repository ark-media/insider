...
// filtering/projection/accumulation; this owns the HTTP + loop invariants
// shared by every fetcher below.
...
//
// `conversation` is the discussion space among the Fold's eight (announcements,
// ask-share, conversation, events, faqs, get-started, lounge, say-hi).
//
// It is also the space DISCUSS_SPACE_BINDINGS posts newsletter companion
// threads into, so the highlights feed would otherwise fill with one
// auto-generated stub per article. Those posts are subtracted below by the ids
// we recorded when we created them; if this slug ever stops colliding with the
// discuss binding, the subtraction becomes a no-op.
...
async function fetchCircleCommunityFeed(
  ...
  if (spaceId === null) {
    // Say so. A renamed or deleted space must not degrade to an empty feed
