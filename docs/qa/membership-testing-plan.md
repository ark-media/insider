...
synchronization (Simplecast, Beehiiv, Auth0, Circle, Neon DB).
...
change fans out to five systems. Simplecast is the **read-authoritative** signal
for `/api/me` (presence of an SC subscription = paid member, regardless of a stale
JWT claim). Auth0 `app_metadata.tier` is a **mirror**, written *after* SC activation.
...
| User state | Stripe | Simplecast | Beehiiv tier | Auth0 `tier` | Circle | `/api/me` |
|---|---|---|---|---|---|---|
| **Never subscribed** | none | none | none (or free if logged in once) | `free` (or absent) | not in group | `free` |
| **Active paid** | `active`, `cancel_at_period_end=false` | SC sub exists, `autorenew=true`, `ends_at=null` | premium on Members Letter + Ark Daily | `ark-plus-member` | in group | `ark-plus-member` |
| **Cancelled (pending, pre-period-end)** | `active`, `cancel_at_period_end=true`, `cancel_at` set | SC sub exists, `autorenew=false`, `ends_at=<cancel_at>` | **still premium** until period end | **still** `ark-plus-member` | **still** in group | `ark-plus-member` |
| **Reactivated** | `active`, `cancel_at_period_end=false` | SC sub exists, `autorenew=true`, `ends_at=null` | premium | `ark-plus-member` | in group | `ark-plus-member` |
| **Expired / deleted** | `canceled` (or `paused`) | SC sub **deleted** | downgraded to free | `free` | removed from group | `free` |
| **Gift active** | n/a (payment_intent) | SC sub exists | premium | `ark-plus-member` + `gift_expires_at` | in group | `ark-plus-member` |
| **Payment failed (past_due)** | `past_due` | SC sub exists (until deleted) | premium (until deleted) | `ark-plus-member` (until deleted) | in group | `ark-plus-member` |
...
- **Stripe:** subscription object — `status`, `cancel_at_period_end`, `cancel_at`,
  `current_period_end`.
- **Simplecast:** lookup user by email → subscription present? `autorenew`, `ends_at`.
...
|---|---|---|---|
| **SUB-01** | Monthly happy path | Given a new email; When checkout with `plan=monthly` + card `4242`; Then payment succeeds, user redirected to `/?checkout=complete`, and `/api/me` returns `ark-plus-member`. | Full "Active paid" row of §1.1: Stripe `active`; SC sub created `autorenew=true`; Beehiiv premium on both pubs; Auth0 `ark-plus-member`; Circle in group; ledger row present. |
...
| **SUB-05** | Adaptive Pricing currency | Given a non-USD billing country (test override); When checkout; Then localized currency shown and charged. | Stripe subscription currency matches; SC/Beehiiv/Auth0/Circle unaffected (still paid). |
...
| **SUB-10** | Duplicate email already subscribed | Given an already-active paid email; When they start checkout again; Then no duplicate active subscription is silently created (verify intended behavior — block or reuse customer). | No second SC subscription / no double Beehiiv premium; document actual behavior. |
...
SC cancel schedule synced via webhook `customer.subscription.updated`.

| ID | Title | Given / When / Then | Sync assertions |
|---|---|---|---|
| **CXL-01** | Cancel at period end | Given an active paid, logged-in user; When they cancel with a reason; Then Stripe sets `cancel_at_period_end=true` + `cancel_at`; response shows access-until date. | "Cancelled (pending)" row: SC `autorenew=false`, `ends_at=<cancel_at>`; **Beehiiv still premium**; **Auth0 still `ark-plus-member`**; **still in Circle**; `/api/me` still `ark-plus-member`. |
...
| **CXL-04** | Period-end teardown | When `current_period_end` passes (or simulate via Stripe test clock / resend `customer.subscription.deleted`); Then entitlement torn down. | "Expired / deleted" row: SC sub **deleted**; Beehiiv downgraded to free; Auth0 `free`; removed from Circle; `/api/me` = `free`. |
...
`cancel_at_period_end`. SC synced back (`ends_at=null`, `autorenew=true`) via webhook.

| ID | Title | Given / When / Then | Sync assertions |
|---|---|---|---|
| **RACT-01** | Reactivate pending-cancel | Given a paid user who cancelled (CXL-01) and is still before period end; When they reactivate; Then Stripe `cancel_at_period_end=false`, `cancel_at` cleared; response shows next renewal date. | "Reactivated" row: SC `autorenew=true`, `ends_at=null`; Beehiiv premium; Auth0 `ark-plus-member`; in Circle; `/api/me` = `ark-plus-member`. |
| **RACT-02** | Reactivate after full expiry | Given a user whose sub already deleted at period end; When they hit reactivate; Then it fails gracefully (must re-subscribe via checkout, not reactivate). | No partial SC/Beehiiv/Circle re-grant; user directed to checkout. |
| **RACT-03** | Cancel → reactivate → cancel | Rapid toggle within one period; Then final Stripe state matches the last action and all systems reflect it. | After last action, full truth-table row matches; SC `autorenew` toggled correctly each time; no orphaned SC `ends_at`. |
...
- **Simplecast leg:** temporarily set an invalid `SC` token / point SC base URL to a
  blackhole, or use a mock that returns 5xx.
...
|---|---|---|---|---|
| **FAIL-SC-01** | Simplecast | 5xx / timeout on activate | Webhook returns non-2xx → Stripe will **retry**. Since SC is read-authoritative, `/api/me` stays `free` until SC succeeds — confirm user is **not** shown as paid with no SC record. | Stripe retry succeeds → SC created; full row converges. |
| **FAIL-SC-02** | Simplecast | 4xx (e.g. duplicate) on activate | Handler treats as idempotent / desired state (verify it does not crash the whole webhook). | Other legs complete; no retry storm. |
| **FAIL-SC-03** | Simplecast | DELETE fails on teardown | Entitlement teardown blocked for SC; user may still appear paid. | Retry or nightly reconcile removes SC; `/api/me` → `free`. |
...
| **FAIL-AU-01** | Auth0 | mgmt API 5xx / scope revoked on set tier | `app_metadata.tier` not updated → JWT claim stale on next login. Confirm `/api/me` still relies on **SC**, not the stale claim. | Retry/reconcile sets `tier`; new login embeds correct claim. |
...
| **FAIL-DB-01** | DB ledger | Insert to `stripe_webhook_events` fails | If ledger write is the dedup gate, a failed write may cause **reprocessing** on retry. Confirm downstream effects stay idempotent (no double Beehiiv premium, no duplicate SC sub). | On retry, event processed exactly-once in effect. |
| **FAIL-MULTI-01** | Two legs (e.g. Auth0 + Circle) | Both 5xx | Partial fan-out: SC+Beehiiv succeed, Auth0+Circle stale. | Reconcile cron heals both passes; final row converges. |

> For each FAIL case, **record the intermediate (drifted) state** of all five
...
|---|---|---|---|
| **IDEM-01** | Replayed event | When the same `subscription.created` event is delivered twice (Resend); Then the second is deduped via `stripe_webhook_events`. | No second SC sub; no double Beehiiv premium; single ledger row; `/api/me` unchanged. |
| **IDEM-02** | Replayed cancel | Resend `subscription.updated` (cancel) twice; Then SC cancel schedule set once, idempotent. | `ends_at`/`autorenew` correct; no duplicate survey rows. |
...
| **ORDER-02** | created → deleted rapid | Subscribe then immediately delete (test clock); Then teardown wins; no lingering entitlement. | Final "Expired/deleted" row; no residual SC/Beehiiv/Circle access. |
...
cookie, then uses **Simplecast as authoritative**.

| ID | Title | Given / When / Then | Pass criteria |
|---|---|---|---|
| **ME-01** | SC present overrides stale free claim | Given SC sub exists but Auth0 claim says `free` (drift); When `/api/me`; Then returns `ark-plus-member`. | SC wins over stale JWT. |
| **ME-02** | Stale paid claim, no SC | Given JWT claims `ark-plus-member` but no SC sub; When `/api/me`; Then it does **not** grant paid (avoids spoofed/stale upgrade). | Returns `free` (not paid). |
| **ME-03** | First login auto free-subscribe | Given an Auth0-sourced login, no SC, not claiming paid; When `/api/me`; Then idempotent free Beehiiv subscribe (`ensureFreeSubscription`). | One Ark Daily subscribe; repeated calls do not duplicate. |
| **ME-04** | Checkout-source provisioning gap | Given checkout token but no SC sub yet (activation lag); When `/api/me`; Then 401 provisioning gap (not a false paid). | 401; resolves to paid once webhook activates SC. |
...
|---|---|---|
| **GIFT-01** | Gift purchase activates | `payment_intent.succeeded` with `metadata.kind=gift` → SC sub + Auth0 `ark-plus-member` + `gift_expires_at` set; Beehiiv premium; Circle access. |
...
- **Exit criteria for launch:**
  ...
  - [ ] `/api/me` never grants paid without an SC subscription (ME-02, ME-04).
...
1. **Webhook atomicity — mixed, by design.** Per-leg behavior, not all-or-nothing:
   - **SC activation** (the paid product) is the **only hard-fail leg**: if it throws,
   ...
   - **SC cancel-schedule mirror, SC DELETE, Beehiiv** are **soft-fail** (try/catch
     + log) → 200.
...
3. **Duplicate active subscriptions (SUB-10):** guarded at *activation* via
   ...
   billing) while SC stays single. SUB-10 must assert no double Stripe billing.
...
### ⚠️ Highest-severity sync risk — Simplecast over-entitlement

`stripe.ts:712-716` documents that **the reconciler heals Auth0/Circle drift but
never touches Simplecast.** Therefore a permanently-failing **SC DELETE** at period
end leaves the member with **paid feed access and no automatic recovery**. Treat
**FAIL-SC-03** as P0: add monitoring/alerting on SC delete failures and a manual
cleanup runbook. There is no cron that will fix this.
...
|---|---|---|
| ME-01 (SC overrides stale free claim) | ✅ | `server/me.test.ts:312` |
| ME-02 (stale paid claim, no SC → 401) | ✅ | `server/me.test.ts:330` |
...
| SC cancel-schedule mirror (cancel/un-cancel) | ✅ | `server/stripe-webhook.test.ts:216` |
| SC PATCH 404 on reorder → still 200 | ✅ | `server/stripe-webhook.test.ts:275` |
| **FAIL-AU/CR at webhook layer → not 500** | ✅ **[new]** | `server/stripe-webhook.test.ts` (resilience block) |
| **FAIL-SC-03 SC-delete soft-fail → still 200** | ✅ **[new]** | `server/stripe-webhook.test.ts` (resilience block) |
...
| End-to-end checkout → SC → all-systems (real APIs) | ❌ manual | Suite A/E (staging) |
...
**Manual focus:** the failure matrix's *recovery* legs against **real** Auth0/Circle/
Beehiiv/SC, the SC over-entitlement P0 (FAIL-SC-03), dunning timing, and the
double-billing risk (SUB-10) — none of which a unit test can prove end-to-end.
